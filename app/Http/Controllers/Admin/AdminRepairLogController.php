...
class AdminRepairLogController extends Controller
{
    public function index(Request $request)
    {
        ...
        $data['set'] = 'vehicles';
        return view('admin.repair_log.repair_logs', $data);
    }
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $btn = '<a href="/admin/edit_repair_log/' . $row->repair_log_id . '" class="btn btn-primary rounded-circle btn-sm" title="Edit"><i class="fa fa-edit"></i></a>';
    ...
    public function add_repair_log(Request $request)
    {
        if ($request->has('submit')) {
            ...
            if ($validator->fails()) {
                ...
                $ins['repair_log_added_by']   = Auth::guard('admin')->user()->admin_id;
                $ins['repair_log_updated_on'] = date('Y-m-d H:i:s');
                $ins['repair_log_updated_by'] = Auth::guard('admin')->user()->admin_id;
                $ins['repair_log_status']     = 1;

                $add = RepairLog::create($ins);

                if ($add) {
                    return redirect('admin/repair_logs/' . $request->segment(3))->with('success', 'Details Added Successfully');
                }
            }
        }
        ...
        $data['set'] = 'vehicles';
        return view('admin.repair_log.add_repair_log', $data);
    }
    ...
    public function edit_repair_log(Request $request)
    {
        if ($request->has('submit')) {
            ...
            if ($validator->fails()) {
                ...
                $upd['repair_log_updated_by'] = Auth::guard('admin')->user()->admin_id;

                $add = RepairLog::where('repair_log_id', $request->segment(3))->update($upd);

                return redirect('admin/repair_logs/' . $request->vehicle_id)->with('success', 'Details Updated Successfully');
            }
        }
        ...
        $data['set'] = 'vehicles';
        return view('admin.repair_log.edit_repair_log', $data);
    }
