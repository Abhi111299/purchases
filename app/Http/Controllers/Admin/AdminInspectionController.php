...
class AdminInspectionController extends Controller
{
    public function index(Request $request)
    {
        ...
        $data['set'] = 'vehicles';
        return view('admin.inspection.inspections',$data);
    }
        ...
        if($request->ajax())
        {
            ...
            return Datatables::of($data)
                    ...
                    ->addColumn('action', function($row){
                        ...
                        $btn = '<a href="/admin/edit_inspection/'.$row->inspection_id. '" class="btn btn-primary rounded-circle btn-sm" title="Edit"><i class="fa fa-edit"></i></a>';
    ...
    public function add_inspection(Request $request)
    {
        ...
        {
            $rules = ['inspection_date' => 'required',
                      'inspection_odometer' => 'required',
                      'inspection_inspected' => 'required'
                     ];

            $messages = ['inspection_date.required' => 'Please Select Date',
            ...
            else
            {
                ...
                $ins['inspection_added_by']    = Auth::guard('admin')->user()->admin_id;
                $ins['inspection_updated_on']  = date('Y-m-d H:i:s');
                $ins['inspection_updated_by']  = Auth::guard('admin')->user()->admin_id;
                ...
                if($add)
                {
                    return redirect('admin/inspections/'.$request->segment(3))->with('success','Details Added Successfully');
                }
            }
        }
        ...
        $data['set'] = 'vehicles';
        return view('admin.inspection.add_inspection',$data);
    }
    ...
    public function edit_inspection(Request $request)
    {
        if($request->has('submit'))
        {
            ...
            else
            {
                ...
                $upd['inspection_updated_by']  = Auth::guard('admin')->user()->admin_id;

                $add = Inspection::where('inspection_id',$request->segment(3))->update($upd);

                return redirect('admin/inspections/'.$request->vehicle_id)->with('success','Details Updated Successfully');
            }
        }
        ...
        $data['set'] = 'vehicles';
        return view('admin.inspection.edit_inspection',$data);
    }
