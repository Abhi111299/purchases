...
class AdminAccidentController extends Controller
{
    public function index(Request $request)
    {
        ...
        $data['set'] = 'vehicles';
        return view('admin.accident.accidents', $data);
    }
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $btn = '<a href="/admin/edit_accident/' . $row->accident_id . '" class="btn btn-primary rounded-circle btn-sm" title="Edit"><i class="fa fa-edit"></i></a>';
    ...
    public function add_accident(Request $request)
    {
        if ($request->has('submit')) {
            ...
            if ($validator->fails()) {
                ...
                $ins['accident_added_by']   = Auth::guard('admin')->user()->admin_id;
                $ins['accident_updated_on'] = date('Y-m-d H:i:s');
                $ins['accident_updated_by'] = Auth::guard('admin')->user()->admin_id;
                ...
                if ($add) {
                    return redirect('admin/accidents/' . $request->segment(3))->with('success', 'Details Added Successfully');
                }
            }
        }
        ...
        $data['set'] = 'vehicles';
        return view('admin.accident.add_accident', $data);
    }
    ...
    public function edit_accident(Request $request)
    {
        if ($request->has('submit')) {
            ...
            if ($validator->fails()) {
                ...
                $upd['accident_updated_by'] = Auth::guard('admin')->user()->admin_id;
                ...
                return redirect('admin/accidents/' . $request->vehicle_id)->with('success', 'Details Updated Successfully');
            }
        }
        ...
        $data['set'] = 'vehicles';
        return view('admin.accident.edit_accident', $data);
    }
