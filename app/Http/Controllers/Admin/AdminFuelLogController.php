...
class AdminFuelLogController extends Controller
{
    public function index(Request $request)
    {
        ...
        $data['set'] = 'vehicles';
        return view('admin.fuel_log.fuel_logs',$data);
    }
        ...
        if($request->ajax())
        {
            ...
            return Datatables::of($data)
                    ...
                    ->addColumn('action', function($row){
                        ...
                        $btn = '<a href="/admin/edit_fuel_log/'.$row->fuel_log_id. '" class="btn btn-primary rounded-circle btn-sm" title="Edit"><i class="fa fa-edit"></i></a>';
    ...
    public function add_fuel_log(Request $request)
    {
        if($request->has('submit'))
        {
            ...
            $messages = ['fuel_log_date.required' => 'Please Select Date',
            ...
            else
            {
                ...
                $ins['fuel_log_added_by']   = Auth::guard('admin')->user()->admin_id;
                $ins['fuel_log_updated_on'] = date('Y-m-d H:i:s');
                $ins['fuel_log_updated_by'] = Auth::guard('admin')->user()->admin_id;
                ...
                if($add)
                {
                    return redirect('admin/fuel_logs/'.$request->segment(3))->with('success','Details Added Successfully');
                }
            }
        }
        ...
        $data['set'] = 'vehicles';
        return view('admin.fuel_log.add_fuel_log',$data);
    }
    ...
    public function edit_fuel_log(Request $request)
    {
        if($request->has('submit'))
        {
            ...
            else
            {
                ...
                $upd['fuel_log_updated_by'] = Auth::guard('admin')->user()->admin_id;

                $add = FuelLog::where('fuel_log_id',$request->segment(3))->update($upd);

                return redirect('admin/fuel_logs/'.$request->vehicle_id)->with('success','Details Updated Successfully');
            }
        }
        ...
        $data['set'] = 'vehicles';
        return view('admin.fuel_log.edit_fuel_log',$data);
    }
