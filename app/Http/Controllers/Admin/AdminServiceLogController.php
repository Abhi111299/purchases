...
class AdminServiceLogController extends Controller
{
    public function index(Request $request)
    {
        ...
        $data['set'] = 'vehicles';
        return view('admin.service_log.service_logs', $data);
    }
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $btn = '<a href="/admin/edit_service_log/' . $row->service_log_id . '" class="btn btn-primary rounded-circle btn-sm" title="Edit"><i class="fa fa-edit"></i></a>';
    ...
    public function add_service_log(Request $request)
    {
        if ($request->has('submit')) {
            ...
            if ($validator->fails()) {
                ...
                $ins['service_log_added_by']   = Auth::guard('admin')->user()->admin_id;
                $ins['service_log_updated_on'] = date('Y-m-d H:i:s');
                $ins['service_log_updated_by'] = Auth::guard('admin')->user()->admin_id;
                $ins['service_log_status']     = 1;

                $add = ServiceLog::create($ins);

                if ($add) {
                    return redirect('admin/service_logs/' . $request->segment(3))->with('success', 'Details Added Successfully');
                }
            }
        }
        ...
        $data['set'] = 'vehicles';
        return view('admin.service_log.add_service_log', $data);
    }
    ...
    public function edit_service_log(Request $request)
    {
        if ($request->has('submit')) {
            ...
            if ($validator->fails()) {
                ...
                $upd['service_log_updated_by'] = Auth::guard('admin')->user()->admin_id;

                $add = ServiceLog::where('service_log_id', $request->segment(3))->update($upd);

                return redirect('admin/service_logs/' . $request->vehicle_id)->with('success', 'Details Updated Successfully');
            }
        }
        ...
        $data['set'] = 'vehicles';
        return view('admin.service_log.edit_service_log', $data);
    }
