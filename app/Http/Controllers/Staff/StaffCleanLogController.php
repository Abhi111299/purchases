...
class StaffCleanLogController extends Controller
{
    public function index(Request $request)
    {
        ...
        $data['set'] = 'vehicles';
        return view('staff.cleaning_log.cleaning_logs', $data);
    }
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $btn = '<a href="/staff/edit_cleaning_log/' . $row->cleaning_log_id . '" class="btn btn-primary rounded-circle btn-sm" title="Edit"><i class="fa fa-edit"></i></a>';
    ...
    public function add_cleaning_log(Request $request)
    {
        if ($request->has('submit')) {
            ...
            if ($validator->fails()) {
                ...
                $ins['cleaning_log_added_by']   = Auth::guard('staff')->user()->staff_id;
                $ins['cleaning_log_updated_on'] = date('Y-m-d H:i:s');
                $ins['cleaning_log_updated_by'] = Auth::guard('staff')->user()->staff_id;
                $ins['cleaning_log_status']     = 1;

                $add = CleaningLog::create($ins);

                if ($add) {
                    return redirect('staff/cleaning_logs/' . $request->segment(3))->with('success', 'Details Added Successfully');
                }
            }
        }

        $data['set'] = 'vehicles';
        return view('staff.cleaning_log.add_cleaning_log', $data);
    }
    ...
    public function edit_cleaning_log(Request $request)
    {
        if ($request->has('submit')) {
            ...
            if ($validator->fails()) {
                ...
                $upd['cleaning_log_updated_by'] = Auth::guard('staff')->user()->staff_id;

                $add = CleaningLog::where('cleaning_log_id', $request->segment(3))->update($upd);

                return redirect('staff/cleaning_logs/' . $request->vehicle_id)->with('success', 'Details Updated Successfully');
            }
        }
        ...
        $data['set'] = 'vehicles';
        return view('staff.cleaning_log.edit_cleaning_log', $data);
    }
