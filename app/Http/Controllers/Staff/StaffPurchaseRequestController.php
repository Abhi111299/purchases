...
class StaffPurchaseRequestController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('staff.purchase.purchase_request', $data);
    }
    ...
    public function add_purchase_request(Request $request)
    {
        if ($request->has('submit')) {
            ...
            if ($validator->fails()) {
                ...
                $ins['staff_id']       = Auth::guard('staff')->user()->staff_id;
                    ...
                    $get_staffs = Staff::whereIn('staff_id', $requestToApproval)->get(); // dd($get_staffs, $requestToApproval);  
                        foreach ($get_staffs as $get_staff) {
                            $emailData = [
                                ...
                                'username' => Auth::guard('staff')->user()->staff_fname, // Pass any other dynamic data you need
                            ...
                            $send = Mail::send('staff.mail.apply_purchase_request', $emailData, function ($message) use ($mail, $uname, $subject) {
        ...
        $data['set'] = 'add_purchase_request';
        return view('staff.purchase.add_purchase_request', $data);
    }
    ...
    public function get_purchase_request(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data = PurchaseRequest::getDetailsStaff();
            ...
            return Datatables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    //     $btn .= '<a href="/staff/edit_purchase_request/' . $row->id . '" class="btn btn-primary rounded-circle btn-sm" title="Edit"><i class="fa fa-edit"></i></a> ';
                    // }

                    // if(!in_array('11',Request()->modules))
                    // {
                    //     $btn .= '<a href="/staff/delete_purchase_request/'.$row->id .'" class="btn btn-danger btn-sm rounded-circle" title="Delete" onclick="confirm_msg(event)"><i class="fa fa-trash"></i></a>';
                    // }
    ...
    public function edit_purchase_request(Request $request, $id)
    {
        if ($request->has('submit')) {
            ...
            if ($validator->fails()) {
                ...
                $purchaseRequest->staff_id       = Auth::guard('staff')->user()->staff_id;
        ...
        return view('staff.purchase.edit_purchase_request', $data);
