...
class StaffTripLogController extends Controller
{
    public function index(Request $request)
    {
        ...
        $data['set'] = 'vehicles';
        return view('staff.trip_log.trip_logs', $data);
    }
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $btn = '<a href="/staff/edit_trip_log/' . $row->trip_log_id . '" class="btn btn-primary rounded-circle btn-sm" title="Edit"><i class="fa fa-edit"></i></a>';
    ...
    public function add_trip_log(Request $request)
    {
        if ($request->has('submit')) {
            ...
            if ($validator->fails()) {
                ...
                $ins['trip_log_added_by']   = Auth::guard('staff')->user()->staff_id;
                $ins['trip_log_updated_on'] = date('Y-m-d H:i:s');
                $ins['trip_log_updated_by'] = Auth::guard('staff')->user()->staff_id;
                ...
                if ($add) {
                    return redirect('staff/trip_logs/' . $request->segment(3))->with('success', 'Details Added Successfully');
                }
            }
        }
        ...
        $data['set'] = 'vehicles';
        return view('staff.trip_log.add_trip_log', $data);
    }
    ...
    public function edit_trip_log(Request $request)
    {
        if ($request->has('submit')) {
            ...
            if ($validator->fails()) {
                ...
                $upd['trip_log_updated_by'] = Auth::guard('staff')->user()->staff_id;

                $add = TripLog::where('trip_log_id', $request->segment(3))->update($upd);

                return redirect('staff/trip_logs/' . $request->vehicle_id)->with('success', 'Details Updated Successfully');
            }
        }
        ...
        $data['set'] = 'vehicles';
        return view('staff.trip_log.edit_trip_log', $data);
    }
