...
class StaffInsuranceController extends Controller
{
    public function index(Request $request)
    {
        ...
        $data['set'] = 'vehicles';
        return view('staff.insurance.insurances', $data);
    }
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($data)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $btn = '<a href="/staff/edit_insurance/' . $row->insurance_id . '" class="btn btn-primary rounded-circle btn-sm" title="Edit"><i class="fa fa-edit"></i></a>';
    ...
    public function add_insurance(Request $request)
    {
        if ($request->has('submit')) {
            ...
            if ($validator->fails()) {
                ...
                $ins['insurance_added_by']   = Auth::guard('staff')->user()->staff_id;
                $ins['insurance_updated_on'] = date('Y-m-d H:i:s');
                $ins['insurance_updated_by'] = Auth::guard('staff')->user()->staff_id;
                $ins['insurance_status']     = 1;

                $add = Insurance::create($ins);

                if ($add) {
                    return redirect('staff/insurances/' . $request->segment(3))->with('success', 'Details Added Successfully');
                }
            }
        }

        $data['set'] = 'vehicles';
        return view('staff.insurance.add_insurance', $data);
    }
    ...
    public function edit_insurance(Request $request)
    {
        if ($request->has('submit')) {
            ...
            if ($validator->fails()) {
                ...
                $upd['insurance_updated_by'] = Auth::guard('staff')->user()->staff_id;

                $add = Insurance::where('insurance_id', $request->segment(3))->update($upd);

                return redirect('staff/insurances/' . $request->vehicle_id)->with('success', 'Details Updated Successfully');
            }
        }
        ...
        $data['set'] = 'vehicles';
        return view('staff.insurance.edit_insurance', $data);
    }
